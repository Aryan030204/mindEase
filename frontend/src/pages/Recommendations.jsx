...
const actionButtons = [
  { action: "accepted", label: "Accept", icon: CheckCircle2, className: "border-emerald-200 text-emerald-700 hover:bg-emerald-50 dark:border-emerald-500/30 dark:text-emerald-200 dark:hover:bg-emerald-500/10" },
  { action: "completed", label: "Complete", icon: Sparkles, className: "border-sky-200 text-sky-700 hover:bg-sky-50 dark:border-sky-500/30 dark:text-sky-200 dark:hover:bg-sky-500/10" },
  { action: "ignored", label: "Ignore", icon: XCircle, className: "border-slate-200 text-slate-600 hover:bg-slate-50 dark:border-border dark:text-muted-foreground dark:hover:bg-muted/40" },
]
          ...
          <CardContent className="space-y-4">
            {recommendations?.activities?.length ? (
              recommendations.activities.map((activity) => (
                <div key={activity._id} className="rounded-3xl border border-border/70 bg-muted/30 p-5">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <h3 className="text-lg font-semibold text-foreground">{activity.personalizedTitle}</h3>
                      <p className="mt-2 text-sm leading-6 text-muted-foreground">{activity.personalizedDescription}</p>
                      <p className="mt-3 text-xs uppercase tracking-[0.16em] text-muted-foreground">
                        {activity.relevanceContext}
                      </p>
                    </div>
                    <span className="rounded-full bg-background px-3 py-1 text-xs font-semibold text-foreground shadow-sm dark:bg-muted/70">
              ...
              <p className="text-sm text-muted-foreground">No adaptive set is ready yet. Start by logging your mood.</p>
            )}
          </CardContent>
        </Card>
          ...
          <Card className="border-border/70 bg-card/95">
            ...
            <CardContent className="space-y-3">
              {dailySuggestions.map((item) => (
                <div key={item.activityType} className="rounded-2xl border border-border/70 p-3">
          ...
          <Card className="border-border/70 bg-card/95">
            ...
            <CardContent className="space-y-3">
              {recentHistory.map((item) => (
                <div key={item._id} className="rounded-2xl bg-muted/30 p-3">
                  <div className="flex items-center justify-between gap-3">
                    <p className="text-sm font-medium text-foreground">
                      {item.activities?.[0]?.personalizedTitle || "Recommendation cycle"}
                    </p>
                    <span className="inline-flex items-center gap-1 text-xs uppercase tracking-[0.16em] text-muted-foreground">
